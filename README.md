...
Optionally, you can also now play with modern controls (WASD keys for movement and mouse for turning), as well as using the TAB key to check the map (ala Doom) or your collection progress (ala Quake). For those who consider this "cheating" or otherwise an inauthentic experience, a pure version with just the MPU-401 support is also available.
